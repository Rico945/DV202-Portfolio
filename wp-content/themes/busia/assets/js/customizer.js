jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					radiantthemesAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			radiantthemesGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				radiantthemesGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			radiantthemesGetAllInputs($(this).parent().parent().parent());
		}
	});
	// Add new item
	$('.customize-control-sortable-repeater-add').click(function(event) {
		event.preventDefault();
		radiantthemesAppendRow($(this).parent());
		radiantthemesGetAllInputs($(this).parent());
	});
	...
	function radiantthemesAppendRow($element, defaultValue = '') {
	...
	function radiantthemesGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
		radiantthemesGetAllImageCheckboxes($(this).parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function radiantthemesGetAllImageCheckboxes($element) {
	...
	$( ".pill_checkbox_control .sortable" ).sortable({
		...
		update: function( event, ui ) {
			radiantthemesGetAllPillCheckboxes($(this).parent());
		}
	});
	$('.pill_checkbox_control .sortable-pill-checkbox').on('change', function () {
		radiantthemesGetAllPillCheckboxes($(this).parent().parent().parent());
	});
	// Get the values from the checkboxes and add to our hidden field
	function radiantthemesGetAllPillCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.radiantthemesfontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.radiantthemesfontslist[index].variants, function(val, text) {
			elementFontWeight.append(
				$('<option></option>').val(text).html(text)
			);
		});
		// Update the font category based on the selected font
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.radiantthemesfontslist[index].category);
		radiantthemesGetAllSelects($(this).parent().parent());
	});
	$('.google_fonts_select_control select').on('change', function() {
		radiantthemesGetAllSelects($(this).parent().parent());
	});
	$('.typography-group input').on('change', function() {
		radiantthemesGetAllSelects($(this).parent().parent().parent());
	});
	$('.typography-group select').on('change', function() {
		radiantthemesGetAllSelects($(this).parent().parent().parent());
	});
	function radiantthemesGetAllSelects($element) {
		var selectedFont = {
			font: $element.find('.google-fonts-list').val(),
			fontweight: $element.find('.google-fonts-fontweight-style').val(),
			textalign: $element.find('.google-fonts-text-align').val(),
			texttransform: $element.find('.google-fonts-text-transform').val(),
			textsize: $element.find('.google-fonts-text-size').val(),
			textsizeunit: $element.find('.google-fonts-text-size-unit').val(),
			textlineheight: $element.find('.google-fonts-text-line-height').val(),
			lineheightunit: $element.find('.google-fonts-line-height-unit').val(),
			textletterspacing: $element.find('.google-fonts-text-letter-spacing').val(),
			letterspacingunit: $element.find('.google-fonts-letter-spacing-unit').val(),
			textmargintop: $element.find('.google-fonts-text-margin-top').val(),
			textmargintopunit: $element.find('.google-fonts-margin-top-unit').val(),
			textmarginright: $element.find('.google-fonts-text-margin-right').val(),
			textmarginrightunit: $element.find('.google-fonts-margin-right-unit').val(),
			textmarginbottom: $element.find('.google-fonts-text-margin-bottom').val(),
			textmarginbottomunit: $element.find('.google-fonts-margin-bottom-unit').val(),
			textmarginleft: $element.find('.google-fonts-text-margin-left').val(),
			textmarginleftunit: $element.find('.google-fonts-margin-left-unit').val(),
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].radiantthemestinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].radiantthemestinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].radiantthemesmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['radiantthemes-upsell'] = api.Section.extend( {
